...
import { View, StyleSheet, Text, Modal, TextInput, ScrollView, Platform, TouchableOpacity, TouchableWithoutFeedback, Switch, FlatList, Alert } from "react-native"
...
export function RepeatingEditScreen({ navigation, route }) {

    const { colors } = useTheme()
    const goal = route.params.goal
    const updateRepeatingGoal = useGoalStore((store) => store.updateRepeatingGoal)
    const removeGoal = useGoalStore((store) => store.removeGoal)
    const removeCompletedGoals = useGoalStore((store) => store.removeCompletedGoals)

    const [title, setTitle] = useState(goal.title)
    const [tempo, setTempo] = useState(goal.amount)
    const [temp, setTemp] = useState(1)
    const [tempModalIsOpen, setTempModalIsOpen] = useState(false)
    const frequencyList = ['Daily', 'Weekly', 'Monthly', 'Yearly']
    const [frequency, setFrequency] = useState(frequencyList.indexOf(goal.occurrence_level))
    ...
    const [completeOnList, setCompleteOnList] =useState(goal.complete_on)
    const [reminderEnabled, setReminderEnabled] = useState(goal.reminder)
...
4
    ...
    async function executeDelete() {
        ...
        await fetch(`${EXPO_PUBLIC_URL}/goal?uid=${auth.currentUser.uid}&goal_id=${goal.goal_id}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + idToken
        ...
        .then((result) => {
            if (result.ok) {
                removeGoal(goal.goal_id)
                removeCompletedGoals(goal.goal_id)
                navigation.navigate("GoalHome");
            }
            else {
    ...
    return (
        ...
        <   View style={{flexDirection: "row", alignItems: "center"}}>
                ...
                <Ionicons name={"trash-outline"} color={"red"} size={25} style={{marginLeft: 10}} onPress={deleteGoal} />
            </View>
            ...
            <TouchableOpacity onPress={() => updateGoal()} style={{backgroundColor: "#5468ff", borderRadius: 10, height: 50, justifyContent: "center", alignItems: "center"}} >
