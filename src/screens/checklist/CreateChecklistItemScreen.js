...
export function CreateChecklistItemScreen({ navigation }) {
    ...
    const [date, setDate] = useState(new Date(moment().endOf("D")))
    const [time, setTime] = useState(new Date(moment().endOf("D")))
    ...
    async function insertTodo() {
        ...
        .then((response) => {
            if (response.ok) {
                checklistObj.is_completed = false
                updateStats("total_checklist")
